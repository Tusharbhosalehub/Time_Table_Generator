...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "lamp_innovative"), "SELECT * FROM subjects ");
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "lamp_innovative"),
...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "lamp_innovative"), "SELECT * FROM teachers ");       
...
for ($I = 0; $I < $subjects_count * 4; $I++) {
    ...
    $year = ($sem + 1) / 2;
    $classroom_query = mysqli_query(mysqli_connect("localhost", "root", "", "lamp_innovative"),
...
for ($i = 3; $i < 9; $i += 2) {
    ...
    for ($j = 0; $j < 6; $j++) {    
        $query = "UPDATE $database_name SET period1='" . $subjectslots[$i][$j][0] . "<br>" . $aliasslots[$i][$j][0][0] . "',
                  ...
                  WHERE day='" . $days[$j] . "' "; 
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "lamp_innovative"), $query);
    }
}
